...
void DMXwifiConfig::initConfig(void) {
  ...
  _wifi_config->ap_address    = IPAddress(192,168,4,1);         // ip address of access point
  _wifi_config->ap_gateway    = IPAddress(192,168,4,1);
  _wifi_config->ap_subnet     = IPAddress(255,255,255,0);       // match what is passed to dchp connection from computer
  _wifi_config->sta_address   = IPAddress(192,168,1,250);       // station's static address for STATIC_MODE
  _wifi_config->sta_gateway   = IPAddress(192,168,1,1);
  _wifi_config->sta_subnet    = IPAddress(255,255,255,0);
  ...
  _wifi_config->artnet_portaddr_lo = 1;                         // Default universe is 1 (net 0 subnet 0 universe 1)
  _wifi_config->artnet_portaddr_hi = 0;
  _wifi_config->device_address  = 1;
  strcpy((char*)_wifi_config->node_name, "ESP-DMX-WiFi");
  _wifi_config->input_address = IPAddress(192,168,4,255);
}
...
uint8_t DMXwifiConfig::setupWiFi(IndicateActivityCallback indicateConnecting) {
  ...
  if ( APMode() ) {                      // WiFi startup
    ...
    if (WiFi.status() != WL_CONNECTED) {    // Connection to Station failed, start with default configuration (AP mode)
      initConfig();                         // initialize and but do not store in EEPROM
      rv = LX_AP_MODE;
      WiFi.mode(WIFI_AP);
      WiFi.softAP(SSID());
      WiFi.softAPConfig(apIPAddress(), apGateway(), apSubnet());
